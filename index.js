...
const toPlain = (doc) => {
	...
	delete obj._id;
	delete obj.__v;
	// delete obj.publicId; // Comentado para permitir que publicId llegue al frontend si es necesario
	return obj;
};
...
app.post("/admin/gallery/sign", isAuth, (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, message: "No se pudo generar la firma" });
	}
});
...
app.post("/admin/edit/:id", isAuth, async (req, res) => {
	try {
		...
		if (!current) return res.status(404).send("Producto no encontrado");
		...
		// Si se envía un nuevo publicId y es diferente al actual, borrar el anterior
		if (
			req.body.publicId &&
			current.publicId &&
			current.publicId !== req.body.publicId
		) {
			await destroyCloudinaryAsset(current.publicId, "image");
		}
...
app.post("/admin/delete/:id", isAuth, async (req, res) => {
	try {
		...
		if (product?.publicId) {
			await destroyCloudinaryAsset(product.publicId, "image");
		}
...
app.post("/admin/gallery/add-direct", isAuth, async (req, res) => {
	try {
		const { fileUrl, description, publicId, resourceType, format, type } =
			req.body || {};
		if (!fileUrl || !publicId || !description) {
			return res.status(400).json({
				success: false,
				message: "fileUrl, publicId y description son requeridos",
			});
		}
		...
		res.json({
			...
			item: toPlain(newItem),
		});
	} catch (err) {
		console.error("Error creando ítem de galería:", err);
		res
			.status(500)
			.json({ success: false, message: "Error al crear ítem de galería" });
	}
});
...
app.put("/admin/gallery/edit-direct/:id", isAuth, async (req, res) => {
	try {
		...
		const updated = await GalleryItem.findByIdAndUpdate(req.params.id, update, {
			new: true,
		});
		...
		res.json({
			...
			item: toPlain(updated),
		});
	} catch (err) {
		console.error("Error editando archivo:", err);
		res
			.status(500)
			.json({ success: false, message: "Error al editar archivo" });
	}
});
...
app.post(
	...
	async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ success: false, message: "Error al agregar archivo" });
		}
	}
);
	...
	async (req, res) => {
		try {
			...
			if (
				req.file &&
				current.publicId &&
				current.publicId !== update.publicId
			) {
				await destroyCloudinaryAsset(current.publicId, current.type);
			}
